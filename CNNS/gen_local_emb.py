...
def RunExp_local(args, dataset, data, Net, percls_trn, val_lb,  institution_mask):
    ...
    gnn_net, data = gnn_net.to(device), data.to(device)

    optimizer = torch.optim.Adam(gnn_net.parameters(),
                                 lr=args.lr,
                                 weight_decay=args.weight_decay)
    def train():
        gnn_net.train()
        optimizer.zero_grad()
        out = gnn_net(data)[data.train_mask]
    ...
    def test():
        gnn_net.eval()
        logits, accs, losses, preds = gnn_net(data), [], [], []
    ...
    for epoch in range(args.epochs):
        ...
        if val_loss < best_val_loss:
            ...
            best_model_wts = copy.deepcopy(gnn_net.state_dict())
    ...
    gnn_net.load_state_dict(best_model_wts)
    ...
    if True:
        # 更新嵌入（无需存center-GNN的嵌入）
        if args.localIndex is not None:
            local_feat_emb = gnn_net(data, saveEmb=True)
