...
def getArgs():
    ...
    ap.add_argument('--dataset', type=str, default='twitter_c3', help="Dataset string")
...
if __name__ == '__main__':
    ...
    """以下参数需要修改"""
    args.dataset = 'twitter'  # ['japan','region785', 'state360']
    args.model = 'GAT'
    args.horizon = 7
    args.clientNum = 3
    args.dir_name = '{}_{}_w20h{}c{}'.format(args.model, args.dataset, args.horizon, args.clientNum)  # 文件夹名称, 为不同数据集用不同的文件夹
    #args.institueion_idx = None  # 选择机构，None表示global视图(但是这边无需为global生成嵌入）
    """以上参数需要修改"""
