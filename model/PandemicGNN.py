...
from torch_geometric.nn import GATConv, APPNP, GCNConv, TransformerConv, SAGEConv
import os
#from utils import *
...
class PandemicGNN_SAGE(nn.Module):
    def __init__(self, args, data):
        super().__init__()
        # arguments setting
        self.adj = data.adj
        ...
        # self.GNNBlock1 = GATConv(in_channels=self.hidR, out_channels=self.hidR, heads=self.heads)
        # self.GNNBlock2 = GATConv(in_channels=self.hidR*self.heads, out_channels=self.hidR)

        self.GNNBlock1 = SAGEConv(in_channels=self.hidR, out_channels=self.hidR)
        self.GNNBlock2 = SAGEConv(in_channels=self.hidR, out_channels=self.hidR)


        self.output = nn.Linear(self.hidR*2 , self.hidR)
        self.output2 = nn.Linear(self.hidR, 1)
    ...
    def forward(self, x,  saveEmb=False, shareEmb = None):
        ...
        node_state_temp = torch.zeros(feat_emb.shape, device=torch.device(self.device))
        for i in range(batch_size):
            node_state_temp[i, :, :] = self.GNNBlock1(feat_emb[i, :, :], self.edge_index)

        node_state = torch.zeros(feat_emb.shape, device=torch.device(self.device))
        for i in range(batch_size):
            node_state[i, :, :] = self.GNNBlock2(node_state_temp[i, :, :], self.edge_index)
